...
function buildSystemPrompt(ctx: AgentContext): string {
  const lines: string[] = [
    ...
    '{"type":"generate_work_plan","planName":"<short title>","brief":"<works description>","startDate":"YYYY-MM-DD","endDate":"YYYY-MM-DD","durationDays":<number>,"mode":"new"|"update"} — build a realistic work plan timeline with finish-to-start activity dependencies. Requires an active project. It plans from the project\'s BOQ when one exists; when the project has NO BOQ, include a "brief" describing the works (from what the user tells you, or inferred from the project title/type) and it plans from that instead. startDate plus either endDate or durationDays calibrate the timeline: take them from the project snapshot (start/end dates) when filled; otherwise ask the user for the start date and either the end date or the duration BEFORE acting. Use mode "update" when the user wants to rebuild/reschedule the work plan that is already open, "new" (default) to create a fresh plan.',
    ...
    "- generate_work_plan with NO BOQ: if the project has no BOQ (hasBOQ is false), you must supply a \"brief\" describing the works so it has something to plan. If the project title/type makes the works obvious (e.g. 'Package 1 - Daynile Road' → road construction), infer a concise brief and proceed. If it's genuinely unclear, ask the user one question to describe the works first, then emit with the brief. Don't refuse just because there's no BOQ.",
