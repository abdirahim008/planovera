-- Admin-set account type: make admin_set_organization_subscription keep an
-- organization's account TYPE (organizations.personal) in sync with the chosen
-- plan's audience. Individual plan -> personal = true (single seat); organization
-- plan -> personal = false (admin-allocated seats). This lets a platform admin
-- flip a registered user between Individual and Organization from the Billing Ops
-- panel, and the INDIVIDUAL/ORG classification follows the selection.
--
